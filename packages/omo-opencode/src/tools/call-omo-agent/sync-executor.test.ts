...
describe("executeSync", () => {
  test("sends sync prompt with question and task tools disabled", async () => {
    ...
    expect(promptInput?.body.agent).toBe("Hang Jebat")
  ...
  test("removes invisible agent characters before sending the sync prompt", async () => {
    ...
    //#then
    const promptInput = recorder.getCapturedInput()
    expect(promptInput?.body.agent).toBe("Gajah Mada - Ultraworker")
  })

  test("#given subagent_type is the lowercase config key 'hephaestus' #when executeSync runs #then prompt receives the registered display name 'Hang Tuah - Deep Agent'", async () => {
    //#given
    const executeSync = await importExecuteSync()
    const deps = createDependencies()
    const toolContext = createToolContext()
    ...
    const promptInput = recorder.getCapturedInput()
    expect(promptInput?.body.agent).toBe("Hang Tuah - Deep Agent")
  })

  test("#given subagent_type is the lowercase config key 'sisyphus-junior' #when executeSync runs #then prompt receives the registered display name 'Pemuda Sumpah'", async () => {
    //#given
    const executeSync = await importExecuteSync()
    const deps = createDependencies()
    const toolContext = createToolContext()
    ...
    //#then
    const promptInput = recorder.getCapturedInput()
    expect(promptInput?.body.agent).toBe("Pemuda Sumpah")
  })

  test("#given subagent_type is already a display name like 'explore' (config key == display name) #when executeSync runs #then prompt receives 'Hang Jebat' unchanged", async () => {
  ...
  test("registers child-session bootstrap and tracked prompt state before sync prompt dispatch", async () => {
    ...
    try {
      ...
      //#then
      expect(observed[0]?.agent).toBe("Hang Jebat")
  ...
  test("strips legacy ZWSP-prefixed agent names from persisted sync prompt body (GH-3259)", async () => {
    ...
    //#then
    expect(recorder.getCapturedInput()?.body.agent).toBe("Hang Tuah - Deep Agent")
  })
})
